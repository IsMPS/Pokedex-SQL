...
public class CrearPokemon {
	...
	private String[] tipos = { "Ninguno", "Acero", "Agua", "Bicho", "Dragon", "Electrico", "Fantasma", "Fuego", "Hada",
	...
	public void Siguiente() {
		existe = false;
		if (!ID.getText().isBlank() && !NombrePok.getText().isBlank() && !PesoPok.getText().isBlank()
				&& !textField_2.getText().isBlank() && !((String) comboBox.getSelectedItem()).equals("Ninguno")) {
			if (!((String) comboBox.getSelectedItem()).equals((String) comboBox2.getSelectedItem())) {
				if ((Integer.valueOf(ID.getText())) > 0 ) {
					...
					JOptionPane.showMessageDialog(frame, "El ID debe ser mayor que 0.");
				}
			} else {
